...
function PlayerBusProxy(masterBus) {
  ...
  this.masterBus.subscribe('*', (message, channel) => {
    if (!this.current) return;
    this.current.bus.send(channel, message, PROXY_GROUP);
  }, PROXY_GROUP);

  this.masterBus.subscribe('players.request-update', () => {
    if (this.current) { 
  ...
  this.masterBus.subscribe('players.select', name => {
...
PlayerBusProxy.prototype.registerBus = function(name, bus) {
  ...
  o.bus.subscribe('*', (message, channel) => {
